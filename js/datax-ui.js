(function () {
    ...
    angular.module('dataxUI', ['dataxUtils'])
        ...
        .directive('dxButton', ['$timeout', 'btnSize', function ($timeout, btnSize) {
            return {
                ...
                template: '<button ng-click="clickBtn()" ng-style="style" ng-class="btnCls" type="{{htmltype}}">\
        ...
        .directive('dxSelect', ['dom', function (dom) {
            return {
                ...
                controller: function ($scope, $element, $compile, $timeout) {
                    if (!$scope.items) {
                        console.log('datax-select component require attribute \'items\'')
                        return
                    }
                    /** 如果model未定义的话，进行初始化 */
            ...
            return {
                ...
                template: '<div>\
                        <span class="datax-calendar-picker" ng-click="openPopup()">\
                            <input readonly="" class="datax-calendar-picker-input datax-input" ng-model="dateStr" placeholder="请选择日期">\
            ...
            return {
                ...
                controller: function ($scope, $element, $http, $timeout) {
                    ...
                    vm.fixHeight = function () {
                        ...
                        vm.tableStyle.scroll.body.maxHeight = vm.scrollHeight
                        vm.tableStyle.fixedLeft.bodyInner.maxHeight = vm.scrollHeight
                    }
                    ...
                    vm.$watch('columns', function (newval) {
                        if (isNotNull(newval) && newval.length > 0) {
                            ...
                            newval.forEach(function (item, index) {
                                ...
                                item.rows.forEach(function (col, i) {
            ...
            <div class="datax-tabs-nav-container">\
                ...
                controller: function ($scope, $element) {
                    ...
                    vm.checkTab = function (item) {
                        ...
                        var bodyEles = getByEle($element[0]).parent().find('div[datax-tab-body]').children()
    ...
    /* #endregion */
    .directive('dxModal', ['dom', function (dom){
        return {
            restrict: 'AE',
            replace: true,
            scope: {
            ...
            template:'<div role="dialog" tabindex="-1" class="datax-modal-wrap " style="z-index: 1000;">\
            <div role="document" class="datax-modal" style="min-width: 320px; max-width: 320px; transform-origin: -125.984px 180.563px 0px;">\
                <div class="datax-modal-content">\
                    <div ng-show="title" class="datax-modal-header ">\
                        <div class="datax-modal-title">\
                            <div class="">{{ title }}</div>\
                        </div>\
                    </div>\
                    <div class="datax-modal-body " >\
                        {{ msg }}\
                    </div>\
                    <div class="datax-modal-footer ">\
                        <button ng-show="type==\'confirm\'" class="datax-btn  datax-btn-default" >\
                            <span> 取消 </span>\
                        </button>\
                        <button class="datax-btn  datax-btn-primary" ng-click="onOK()">\
                            <span> 确定 </span>\
                        </button>\
                    </div>\
                </div>\
            ...
            link: function(scope, element, attrs) {
                ...
                scope.cancelModal = function () {
                    getByEle(document.querySelector('.cdk-overlay-container')).remove()
                }
            }
        }
    }])
